...
- [x] **前端工程初始化（Vue）**
  - [x] 创建前端工程目录（例如 `web-ui/`），初始化 Vue 项目骨架
  - [x] 预置基础路由结构：拓扑视图、流量监控、策略管理、告警中心、蜜罐日志、系统设置
  - [x] 配置统一的 API 客户端模块（封装调用应用层后端的 HTTP / WebSocket）

- [x] **应用层本地持久化基础（DB & ORM）**
  - [x] 选型：首期使用 SQLite 作为默认实现，通过环境变量可切换到 PostgreSQL
  - [x] 新增 `db` 模块，封装 `get_engine()` / `get_session()` / `session_scope()` 等基础设施
  - [x] 在 `create_app()` 中初始化数据库连接，但保持懒加载，避免阻塞无 DB 场景（如纯单元测试）
  - [x] 引入迁移工具（Alembic），配置基础迁移环境并创建初始迁移（用户表、审计表、偏好表）
...
- [x] **向前端转发实时事件**
  - [x] 在应用层后端暴露 WebSocket / SSE 端点，转发控制层事件给前端：
    - [x] 前端订阅的统一事件流（如 `/ws/ui-events`），内部按 `type` 区分事件类别
  - [x] 为告警、蜜罐事件、流量异常事件添加最小必要的字段映射（不做额外业务加工）
...
- [x] **对接控制层策略/动作**
  - [x] 定义从决策等级到策略模板的映射关系（例如：block → 调用控制层流级阻断策略接口）
  - [x] 对接最小闭环：
    - [x] 接收某个 `Flow`（来自控制层或数据采集端）
    - [x] 调用本地推理服务得到结果
    - [x] 根据决策通过 `controller_client` 调用控制层 API（如创建并应用流级策略，或重定向到蜜罐）
...
- [x] **拓扑视图**
  - [x] 通过 `GET /api/topology` 渲染节点/连接图
  - [x] 点击节点/连接时，调用状态接口展示实时指标

- [x] **实时监控与告警面板**
  - [x] 订阅应用层 WebSocket `/ws/ui-events`，展示网络状态更新、流量异常、蜜罐告警
  - [x] 提供告警列表视图，支持按时间/严重级别过滤

- [x] **策略管理界面**
  - [x] 策略列表页：展示策略名称、类型、状态、优先级
  - [x] 策略详情/编辑页：基于 `docs/security-policies.md` 的结构渲染表单字段
  - [x] 支持创建/更新/删除/应用/撤销策略的交互，调用对应的应用层 API

- [x] **蜜罐日志与调查界面**
  - [x] 列表展示蜜罐捕获的交互（来源 IP、请求、响应、时间）
  - [x] 支持按时间范围 / 源 IP 过滤

- [x] **模型与系统信息页面（只读）**
  - [x] 展示当前模型版本、主要特征、阈值配置
  - [x] 展示控制层连接状态、令牌状态概览
...
- [x] **应用层自身鉴权与多租户边界（最小实现）**
  - [x] 为 Web UI 用户增加简单登录与会话管理（与控制层 JWT 解耦）
  - [x] 确保对控制层 API 的调用仅由后端发起，前端不直接暴露控制层地址或令牌
  - [x] 登录态、用户信息、会话信息持久化在应用层数据库中（见「九、应用层本地持久化与用户管理」）
...
- [x] **MVP-1：只读监控**
  - [x] 前端能通过应用层看到当前拓扑和节点/连接状态
  - [x] 能查看历史告警和蜜罐日志

- [x] **MVP-2：AI 检测 + 手动策略**
  - [x] 能在前端手动提交一个 `Flow` 样本给 `/api/detect/flow`，看到检测结果
  - [x] 能在前端创建并应用一个简单的流级阻断策略

- [x] **MVP-3：自动化策略闭环**
  - [x] 控制层推送的流更新事件经应用层触发本地检测
  - [x] 检测判定为高风险时，应用层自动调用控制层接口下发阻断/重定向策略
  - [x] 前端在告警中心和蜜罐日志中可看到对应的自动化响应结果
...
- [x] **数据库选型与基础设施**
  - [x] 使用 SQLite 作为默认实现，连接串例如：`sqlite:///ics_guard_app.db`
  - [x] 通过环境变量支持切换到 PostgreSQL（如 `postgresql+psycopg://...`）
  - [x] 封装数据库访问层（例如 `src/app/db.py`），提供 `get_engine()` / `session_scope()` 等基础工具

- [x] **用户与权限模型（应用层 Web UI 自身）**
  - [x] 定义用户表 `app_users`（仅用于 Web UI，不与控制层用户混用）：
    - [x] 字段示例：`id`, `username`, `password_hash`, `role`, `is_active`, `created_at`, `updated_at`
  - [x] 定义最小角色模型：`admin`, `operator`, `viewer`
  - [x] 定义登录 / 登出 / 刷新会话的后端接口（如 `/api/auth/login` 等），只作用于 Web UI

- [x] **审计日志（应用层行为）**
  - [x] 定义审计表 `app_audit_logs`：
    - [x] 字段示例：`id`, `user_id`, `action`, `resource`, `payload_snapshot`, `ip`, `created_at`
  - [x] 在关键后端 API 调用处记录审计事件（例如：策略创建/更新/应用、手动触发检测等）
  - [x] 提供只读审计查询接口（如 `/api/audit/logs`，可分页、按时间/用户过滤）

- [x] **UI / 应用偏好配置**
  - [x] 定义偏好表 `app_preferences`：
    - [x] 支持全局配置（`scope = global`）与用户级配置（`scope = user`, `user_id`）
    - [x] 使用 `key`, `value`（JSON）、`updated_at` 字段存储配置
  - [x] 用于存储：
    - [x] 告警列表默认过滤条件
    - [x] 默认时间范围、图表刷新间隔等 UI 相关参数
  - [x] 提供简单的读写接口（如 `/api/preferences`）

- [x] **安全边界与最小实现**
  - [x] 应用层 Web UI 登录与控制层 JWT 解耦：应用层不直接向前端暴露控制层凭证
  - [x] 数据库中不存任何控制层 access_token / refresh_token，仅存应用层自己的会话信息
